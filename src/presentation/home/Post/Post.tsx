...
import { savePost } from "./service/post_service";
...
const Post: React.FC<PostCardProps> = ({ post, setPosts, navigation }) => {
    ...
    const navigateToPostDetail = () => {
        navigation.navigate(NavigationPath.POST_DETAIL, { id: post.id })
    }
    ...
    return (
        <CardContainer >
            ...
            <Pressable onPress={navigateToPostDetail}>
                <Description>
                    {stringShortener(post.description, 128)}
                    {
                        post.description.length > 128 &&
                        <TouchableOpacity style={{}} onPress={() => navigateToPostDetail()}>
